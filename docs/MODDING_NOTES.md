...
Aseprite 没有"图层蒙版"这个概念，所以只能**烘进 alpha** —— 这是唯一能让蒙版
遮住的部分真的被遮住的办法。

### 31.1 两处缺陷（第二处此前没被发现）
1. **蒙版数据块被整段跳过**（`readLayerRecord` 里 `seek(tell + maskLength)`）
2. ★**蒙版通道按图层尺寸读取**★ —— 蒙版有**自己的矩形**，通常与图层不同。
   也就是说这些通道**本来就在被错误解码**，与有没有人用它无关。

### 31.2 实现
| 位置 | 内容 |
|---|---|
| `psd.h` | `LayerRecord` 增加 `hasMask` / 蒙版矩形 / `maskDefaultColor` / `maskFlags`，以及 `isMaskDisabled()`（flags bit 1） |
| `decoder.cpp` | 解析 18 字节的蒙版块；`-2`/`-3` 通道改用蒙版矩形的尺寸 |
| `psd_format.cpp` | `onImageScanline` 截获蒙版通道存入独立平面；`onEndLayer` 调 `applyMask()` 乘进 alpha |

**蒙版矩形外**用 `maskDefaultColor` 填充 —— 这是 PSD 对"蒙版没覆盖到的区域"的定义。

**索引色**没有逐像素 alpha 可缩放，只能二值处理（<128 置为透明索引），已注明。

### 31.3 验证（三种情形）
| 情形 | 结果 |
|---|---|
| 蒙版只覆盖左半边、默认色=0 | 左 128 像素不透明、右 128 像素透明，异常 0 |
| 全覆盖的**渐变**蒙版（值=x*17） | alpha 精确等于 `0,17,34,...,255` |
